...
router.get('/dashboard/colegios/:municipio/:nivel', async (req, res) => {
  ...
  //  let colegiosComputados = [];
    ...
    if(nivel === 'presidencial'){
      colegiosComputadosP = await ActaPresidencial.find({}, {_id:0, colegio: 1}); 
   } else if (nivel === 'senatorial'){
      colegiosComputadosS = await ActaSenatorial.find({}, {_id:0, colegio: 1});
   }else if (nivel === 'diputadosd1'){
      colegiosComputadosD1 = await ActaDiputadosD1.find({}, {_id:0, colegio: 1});
   } else if (nivel === 'diputadosd2'){
      colegiosComputadosD2 = await ActaDiputadosD2.find({}, {_id:0, colegio: 1});
   }
...
//    console.log(col);
  ...
  router.get('/dashboard/colegios/:municipio/:nivel/:nombre', async (req, res) => {
    ...
    if(nivel === 'presidencial'){
      ...
      votos = await ActaSenatorial.find({colegio: nombre, municipio: municipio}, {_id: 0, nombre: 1, votos: 1}); 
    }else if (nivel === 'diputadosd1'){
      votos = await ActaDiputadosD1.find({colegio: nombre, municipio: municipio}, {_id: 0, nombre: 1, votos: 1}); 
    }else if (nivel === 'diputadosd2'){
      votos = await ActaDiputadosD2.find({colegio: nombre, municipio: municipio}, {_id: 0, nombre: 1, votos: 1}); 
    }
